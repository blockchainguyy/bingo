...
describe("Bingo Game", function () {
  ...
  it("Can join a game before the minimum join time", async function () {
    ...
    await expect(bingo.draw(gameCount)).to.be.revertedWith("Bingo: game not started");
    await time.increase(minJoinWindow);
    await bingo.draw(gameCount);
    await expect(bingo.connect(player2).joinGame(gameCount)).to.be.revertedWith("Bingo: game in progress");
  });
  ...
  it("Can draw next number after the minimum turn time", async function () {
    ...
    await time.increase(minJoinWindow);
    await bingo.draw(gameCount);
    await expect(bingo.draw(gameCount)).to.be.rejectedWith("Bingo: wait for next turn");
  ...
  it("Can't call bingo with wrong pattern index", async function () {
    ...
    await bingo.connect(owner).createGame();
    gameCount++;
    await expect(bingo.connect(player1).bingo(gameCount, 13, drawnIndexes)).to.be.reverted;
  });
  ...
  it("Can't call bingo without joining a game", async function () {
    ...
    await bingo.connect(owner).createGame();
    gameCount++;
    await expect(bingo.connect(player1).bingo(gameCount, 1, drawnIndexes)).to.be.reverted;
  });
